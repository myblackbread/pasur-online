...
export const PlayingCard = ({ card, onClick, disabled, isPending, isSelected, isCapturedTarget }: PlayingCardProps) => {
    ...
    return (
        <motion.button
            ...
            disabled={disabled}
            className={`relative flex-shrink-0 w-14 h-20 sm:w-16 sm:h-24 md:w-20 md:h-28 rounded-xl shadow-md flex flex-col justify-between p-1.5 sm:p-2 font-bold select-none border-2 transition-colors
...
            ${isPending ? 'opacity-60 scale-95' : 'opacity-100'}`}
        >
